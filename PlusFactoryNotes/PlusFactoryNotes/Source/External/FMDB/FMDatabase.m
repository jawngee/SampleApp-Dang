...
+(void)installDatabase:(NSString *)dbPath force:(BOOL)forced
{
	NSString *appPath=[NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES) objectAtIndex:0];
	appPath=[appPath stringByAppendingPathComponent:[[NSBundle mainBundle] bundleIdentifier]];
	...
	appPath = [appPath stringByAppendingPathComponent:[dbPath lastPathComponent]];
	...
	NSError *error=nil;
	[[NSFileManager defaultManager] copyItemAtPath:dbPath toPath:appPath error:&error];
    
    NSLog(@"DB Install Path: %@",appPath);
	
	if (error!=nil)
		NSLog(@"error:%@",[error localizedDescription]);
}

+(FMDatabase *)databaseFor:(NSString *)dbName
{
	if (databases==nil)
		databases=[[[NSMutableDictionary alloc] init] retain];
	
	FMDatabase *result=[databases objectForKey:dbName];
	...
	appPath = [appPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.db", dbName]];
	
	if ([[NSFileManager defaultManager] fileExistsAtPath:appPath]==NO)
		return nil;
	
	result=[[FMDatabase alloc] initWithPath:appPath];
	[databases setObject:result forKey:dbName];
